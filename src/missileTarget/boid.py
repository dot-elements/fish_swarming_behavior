...
class Boid:
	def __init__(self, x, y, isFlawed, DNA={"flee": random.uniform(0, 1), "protect": random.uniform(0, 1)}):
		...
		self.timeToFlaw = 0
		self.faultStrategy = 0
		self.flawStartingTime = 0
		self.DNA = DNA
	...
	def fleeBehaviour(self, predator, weight=0):
		if weight == 0:
	...
	def avoidObstacles(self, list_obs):
		for obs in list_obs:
			toGoal = self.avoidObs(obs)
			self.acceleration.add(toGoal)
	...
	def avoidObs(self, obsPos):
		steering = Vector()
		obsPos = Vector(obsPos[0], obsPos[1])
		if getDistance(obsPos, self.position) > 45:
			return Vector(0, 0)
		steering.add(obsPos)
		steering = steering - self.position
		steering = steering * -1
...
class Predator(Boid):
	...
	def attackPrey(self, prey):
		...
		toGoal = toGoal * weight # attack weight 0.5
		self.acceleration.add(toGoal)
	...
	def attackBehaviour(self, close_prey):
		...
		toGoal = toGoal * weight  # attack weight 0.4
		self.acceleration.add(toGoal)
